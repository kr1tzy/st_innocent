"""
    st_innocent/api/routes/pages.py
"""

from typing import Annotated
from fastapi import APIRouter, Depends
from fastapi.exceptions import HTTPException
from api.models import Page, Index, Content, PageResponse, JSONException
...
async def get_pages(
    ...
    try:
        ...
        for _, page in enumerate(page_names):
            ...
            title = content.data
            pages.append(Page(_id=page, title=title).dict(by_alias=True))

        # sort
        if _field == "id":
            pages.sort(key=lambda p: p["_id"], reverse=_order == "DESC")
        elif _field == "title":
